...
public class BPingPongShm
  { //private static LinuxIPC ipc = new LinuxIPC();
    ...
    public BPingPongShm (int key, String myName) throws IOException
      { int pongKey = ((int)"pong".hashCode()) ^ key;
        ...
        if (myName.equals("Ping"))
          { yourName = "Pong";
            outStream = new ObjectOutputStream(new BufferedOutputStream(new SharedMemoryOutputStream(pongKey, ARRAY_SIZE)));
            outStream.flush();
            System.out.println(myName + " outStream created.");
            inStream = new ObjectInputStream(new SharedMemoryInputStream(pingKey, ARRAY_SIZE));
            System.out.println(myName + " inStream created.");
          }
        else
          { yourName = "Ping";
            outStream = new ObjectOutputStream(new BufferedOutputStream(new SharedMemoryOutputStream(pingKey, ARRAY_SIZE)));
            outStream.flush();
            System.out.println(myName + " outStream created.");
            inStream = new ObjectInputStream(new SharedMemoryInputStream(pongKey, ARRAY_SIZE));
            System.out.println(myName + " inStream created.");
          }
    ...
    public static void main(String[] args)
      { startSetup = System.currentTimeMillis();
        ...
        int key = "BPingPongShm".hashCode();

        try
          { new BPingPongShm(key, myName).play(rounds);
          }
        catch (Exception exc)
