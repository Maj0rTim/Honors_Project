...
JNIEXPORT jint JNICALL Java_LinuxIPC_ftok (JNIEnv * env, jobject obj, jstring pathname, jchar proj) { 
    ...
    if (iscopy) { retval = ftok(c_pathname, c_proj);
        if (retval == -1) 
...
JNIEXPORT jint JNICALL Java_LinuxIPC_msgget (JNIEnv * env, jobject obj, jint key, jint flag) {
    jint num = msgget(key, flag);
    if (num == -1) { setErrnum(env, obj, errno); }
    return num;
}

JNIEXPORT jint JNICALL Java_LinuxIPC_msgsnd (JNIEnv * env, jobject obj, jint id, jint type, jobject buffer, jint length, jint flags) {
...
JNIEXPORT jint JNICALL Java_LinuxIPC_msgrcv (JNIEnv * env, jobject obj, jint id, jobject buffer, jint length, jint type, jint flags) { 
    void *bufferAddress = (*env)->GetDirectBufferAddress(env, buffer);
    if (bufferAddress == NULL) { return -1; }
    struct message_buffer message
